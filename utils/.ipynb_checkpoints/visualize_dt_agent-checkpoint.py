...
def evaluate_agent(env, model, num_episodes, context_length=20, target_return=None, delay=0.05):
    ...
    # if os.getenv("SAVE_METRICS_NPZ"):
    #     out_path = os.getenv("SAVE_METRICS_NPZ")
    #     print(f"Saving episode returns to {out_path}")
    #     np.savez(out_path,
    #              train_losses=np.array([np.nan]),     # viewer has no training loss
    #              val_returns=np.array(total_rewards)) # one entry per episode
